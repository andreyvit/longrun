...
- More subtasks is better than fewer when each slice is reviewable and testable. Treat each subtask as roughly one reasonable reviewable commit or small PR: if work is expected to touch a larger set of things, could plausibly stand on its own, and has observable testable output, split it into its own subtask.
- Make the direct user ask the first shippable implementation path. Do not combine it with discovered architecture, operational, data, support, cleanup, or behavior work unless that work is technically required to deliver the direct ask.
...
- Significant scope expansion goes through the proposal system first. If it technically blocks the direct ask, write the proposal, then use the user-interview or Ask User path to approve the expansion before planning implementation. If it does not block the direct ask, leave the proposal recorded in `Refactorings` and continue the primary scope; the user can approve or request that work later.
...
- Do not expand the active subtask just because research found a broader security, debuggability, modularity, or simplicity gap. Scope complex changes in only when they were explicitly requested or approved, are a natural part of the request, or technically block useful progress with one reasonable implementation path.
...
- For browser-impact subtasks, include explicit browser verification scope: impacted flows, expected checks, required screenshots, when video evidence is required, whether the repo's browser/app harness mode covers the flow, and whether the harness in-page browser tool is sufficient or the OS-level automation fallback is needed.
