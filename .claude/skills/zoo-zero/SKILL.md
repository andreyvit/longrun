...
- In Zoo Zero, separate task-list items and commits stand in for subtasks. More separate items are better than fewer when each item is reviewable and testable; if work is expected to touch a larger set of things, could plausibly stand on its own, and has observable testable output, make it a separate item and commit.
- Keep the user's direct ask as the primary implementation path. Significant scope expansion requires a proposal and explicit user approval before implementation.
...
- Significant scope expansion goes through the proposal system first. If it technically blocks the direct task, write the proposal, then use the user-interview or Ask User path to approve the expansion before implementation. If it does not block the direct task, leave the proposal recorded in the next report and continue the primary scope; the user can approve or request that work later.
