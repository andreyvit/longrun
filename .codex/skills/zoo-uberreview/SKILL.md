...
Significant scope expansion requires a proposal and explicit user approval before implementation. Significant expansion includes new jobs, persisted state or migrations, new settings, public/API contract changes, broad subsystem behavior changes, operational dashboards/recovery mechanisms, and other work a user would not naturally expect from the direct ask. Tell reviewers to route significant expansion through the proposal system first: if it blocks the direct ask, ask the user through the user-interview or Ask User path after the proposal is written; if it does not block, leave the proposal deferred and continue the primary scope.

Tell reviewers that more subtasks is better than fewer when the slices are reviewable and testable. Treat a subtask as roughly one reasonable reviewable commit or small PR; if a larger set of changes could plausibly stand alone and has observable testable output, reviewers should request separate subtask/commit routing.
